...
Parser::Parser(const char *filepath) {
  ...
  try {
    ...
    if (assembly_file_reader.is_open()) {
      // assembly_content =
      // string(istreambuf_iterator<char>(assembly_file_reader),
...
int Parser::first_pass(void) { return 0; }
...
string Parser::comp() {
  if (instructionType() != C_INSTRUCTION)
    throw std::invalid_argument("can't get comp of an A | L Instruction");
